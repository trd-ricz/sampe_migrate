...
function populate_schedules() {
	...
	$students_v1 = $wpdb->get_results("SELECT display_name FROM $wpdb->users as users, $wpdb->usermeta as meta WHERE meta.meta_key = 'wp_capabilities' AND meta.meta_value LIKE '%student%' AND users.ID = meta.user_id");
...
<link rel="stylesheet" href="<?php echo get_stylesheet_directory_uri(); ?>/inc/custom-meta-box/css/css-meta-box.css">
...
<div id="print_preview">
	<?php for ($x = 0; $x < count($schedules[0]); $x++) : ?>
		<?php include __DIR__.'/populate-data.php'; ?>
	<?php endfor; ?>
</div><!--end print-->
